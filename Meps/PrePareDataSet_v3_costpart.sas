...
TITLE " All expences - Pre/Post ACA Out-of-Pocket Costs, Descriptive Statistics";
	PROC SURVEYMEANS DATA=meps.cost_all_together missing SUM STD MEAN STDERR MEDIAN NOBS;
	CLASS ACA_CAT_OF;
	STRATA VARSTR ;
	CLUSTER VARPSU;
	WEIGHT PERWTF;
...
TITLE "MAMMO + BC OUTPOCKET COMPLIANT";
	PROC SURVEYMEANS DATA=meps.cost_logop_breast (WHERE=(MAMMOG=1)) SUM STD MEAN STDERR MEDIAN NOBS;
	CLASS ACA_CAT_OF;
	STRATA VARSTR ;
	CLUSTER VARPSU;
	WEIGHT PERWTF;
...
TITLE "MAMMO + BC OUTPATIENT COMPLIANT";
	PROC SURVEYMEANS DATA=meps.cost_breast (WHERE=(MAMMOG=1)) SUM STD MEAN STDERR MEDIAN NOBS;
	CLASS ACA_CAT_PAT;
	STRATA VARSTR ;
	CLUSTER VARPSU;
	WEIGHT PERWTF;
...
ods pdf body='F:\Projects\Trinh\MEPS\Code\Cost_outpatient_cervical.pdf';

TITLE "Cervical expences - pre/post ACA Out-of-Patient Costs, Descriptive Statistics";
	PROC SURVEYMEANS DATA=meps.cost_logopt_cervical MEAN STDERR median quartiles NOBS;
	CLASS aca_cat_pat;
	STRATA VARSTR;
	CLUSTER VARPSU;
	WEIGHT PERWTF;
...
TITLE "Dummy variable data set";
data meps_dummy_cervical;
set meps.cost_logopt_cervical;

	/* dummy variables leave one ref out *according to stu (-1 is reference)*/
	if aca_cat_pat = 2 then aca = 1; /*post*/
	else if aca_cat_pat = 1 then aca = 0; /* pre*/

	if agecat = 3 then age = 1;
	else if agecat = 2 then age = 0;
	...
	# newrace = 1 is left out (according to stu)
...
TITLE "Test colon for gllimix";
	PROC GLIMMIX DATA=meps_cervical;
	_variance_ = _mu_ ** 2;
	MODEL outpatient = aca_cat_pat agecat race5 race4 race3 race2 lang haveus insur empsta higheduc / LINK = log S;
	RANDOM _residual_; /* adds scale parameter to variance */
	OUTPUT OUT = meps_cervical PRED = _xb_;
	RUN;

TITLE "data setup for cervical";
	data meps_cervical;
	set meps_cervical meps_dummy_cervical;
	mu = exp(_xb_);
...
TITLE "GEE estimate outpatient under cluster sampling and strata for cervical";
	PROC SURVEYREG data=meps_cervical;
	STRATUM varstr;
	CLUSTER varpsu;
	WEIGHT perwtf;
	MODEL y_ = intercep aca age race5 race4 race3 race2 lan us ins emp educ/noint CLPARM solution;
	RUN;
